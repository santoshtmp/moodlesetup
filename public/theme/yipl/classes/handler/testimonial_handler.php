...
class testimonial_handler
{
            ...
            if ($data->id && ($mform_data->action == 'edit')) {
                ...
                if ($data_exists) {
                    ...
                    if ($status) {
                        $a = new stdClass();
                        $a->name = '"' . $data->name . '" ';
                        $message = get_string('testimonial_updated', 'theme_yipl', $a);
                    }
                }
                ...
                if ($status) {
                    $a = new stdClass();
                    $a->name = '"' . $data->name . '" ';
                    $message =  get_string('testimonial_added', 'theme_yipl', $a);
                }
                $return_url = $save_return_url;
            }
        } catch (\Throwable $th) {
            $message = get_string('testimonial_error_submit', 'theme_yipl');
            $message .= "\n" . $th->getMessage();
        }
        ...
        try {
            ...
            if ($data) {
                ...
                if ($delete) {
                    $a = new stdClass();
                    $a->name = '"' . $data->name . '" ';
                    $message =  get_string('testimonial_delete', 'theme_yipl', $a);
                } else {
                    $message =  get_string('testimonial_error_delete', 'theme_yipl');
                }
            } else {
                $message =  get_string('testimonial_delete_missing', 'theme_yipl');
            }
        } catch (\Throwable $th) {
            $message = get_string('testimonial_error_delete', 'theme_yipl');
            $message .= "\n" . $th->getMessage();
    ...
    public static function edit_form($mform, $id, $return_url)
    {
        ...
        try {
            ...
            if ($data) {
                ...
                $message = get_string('testimonial_missing', 'theme_yipl');
            }
        } catch (\Throwable $th) {
            //throw $th;
    ...
    public static function get_data_in_table($baseurl, int $per_page_data = 12)
    {
        ...
        $action_base_url = "/theme/yipl/page/testimonial/edit.php";
        ...
        $table->no_sorting('image');
        $table->no_sorting('content');
        $table->no_sorting('designation');
    ...
    public static function get_data_in_array(int $per_page_data = 30, $status = "published", $sort = 'id DESC')
    {
        ...
        // sql query
        $sql_query = 'SELECT *        
        FROM {yipl_testimonial} AS testi ' .
            $where_condition_apply . '
