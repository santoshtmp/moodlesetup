...
const toggleManualCompletionState = async (toggleButton) => {
    ...
    // window.console.log('clickkkk btn - ' + toggleType);
    // window.console.log(loadingHtml);
    ...
    try {
        ...
        // auto move to next activity if present when mark as completed
        // execute after 5 seconds
        const next_activity_link = $('.activity-navigation .next-link #next-activity-link').attr('href');
        if (next_activity_link && completed && newToggleButton) {
            window.location.href = next_activity_link;
            // window.console.log('--------------');
        }
