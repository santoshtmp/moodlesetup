...
$PAGE->set_url("$CFG->wwwroot/theme/skilllab/pages/login/csc-lms-login.php");
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    ...
    if ($user) {
        ...
        $local_env = ($site_environment == 2) ? true : false;
        $is_siteadmin = (is_siteadmin($user)) ? true : false;
        // only allow login in local env or for admin role
        if ($local_env || $is_siteadmin) {
            ...
            // } else {
            //     redirect(new moodle_url('/skilllab/admin-login.php'), 'invalid login');
            // }
        } else {
            $errormsg = 'Only admin can login from here';
            $errorcode = 3;
        }
    } else {
        if (empty($errormsg)) {
...
if (empty($SESSION->wantsurl)) {
    ...
    if (
        ...
        strpos($referer, $CFG->wwwroot . '/lms-admin-login') !== 0
...
if (!empty($SESSION->loginerrormsg)) {
    ...
    redirect(new moodle_url('/lms-admin-login'));
}
