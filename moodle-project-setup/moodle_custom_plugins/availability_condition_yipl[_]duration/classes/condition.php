 ...
 *
 * @package availability_yipl_duration
 * @copyright 2014 The Open University
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace availability_yipl_duration;
...
class condition extends \core_availability\condition
{
    ...
    public function __construct($structure)
    {
        ...
        // }
        $this->expectedcompletion = property_exists($structure, 'e') ? (int)$structure->e : 0;
        $this->duration = property_exists($structure, 'duration') ? (int)$structure->duration : 0;
    }
     ...
     */
    public static function get_json(int $cmid, int $duration, int $expectedcompletion): stdClass
    {
        return (object) [
            'cm' => (int)$cmid,
            'duration' => $duration,
    ...
    public function is_available($not, info $info, $grabthelot, $userid): bool
    {
        ...
        if ($cmid && $this->duration && $this->expectedcompletion) {

            // duration in seconds
            $duration_sec = 0;
            if ($this->duration == 1) {
                $duration_sec =  $this->expectedcompletion * 60;
            }
            if ($this->duration == 2) {
                $duration_sec =  $this->expectedcompletion * 60 * 60;
            }
            if ($this->duration == 3) {
                $duration_sec =  $this->expectedcompletion * 60 * 60 * 24;
            }

            // check in db
            global $DB, $USER;
            $duration_info =  $DB->get_record('skl_time_track', ['user_id' => $USER->id, 'course_id' => $info->get_course()->id, 'cmod_id' => $cmid]);
            if ($duration_info) {
                if ($duration_info->duration) {
                    if ($duration_info->duration >= $duration_sec) {
                        $allow = true;
                    } else {
                        $allow = false;
                    }
    ...
    private function load_course_structure(stdClass $course): void
    {
        ...
        if (empty($this->modfastprevious)) {
            $previouscache = cache::make('availability_yipl_duration', 'previous_cache');
    ...
    public function get_description($full, $not, info $info): string
    {
        ...
        if ($this->cmid == self::OPTION_PREVIOUS && $editing) {
            ...
            $modinfo = $info->get_modinfo();
            if (!array_key_exists($cmid, $modinfo->cms) || $modinfo->cms[$cmid]->deletioninprogress) {
                $modname = get_string('missing', 'availability_yipl_duration');
            } else {
        ...
        return 'You spend ' . $this->expectedcompletion . ' ' . $duration_time . ' time duration in ' . $modname;
        // return get_string($str, 'availability_yipl_duration', $modname);
    }
    ...
    public static function completion_value_used($course, $cmid): bool
    {
        ...
        if (!array_key_exists($course->id, self::$modsusedincondition)) {
            ...
            foreach ($modinfo->cms as $othercm) {
                ...
                foreach ($tree->get_all_children('availability_yipl_duration\condition') as $cond) {
